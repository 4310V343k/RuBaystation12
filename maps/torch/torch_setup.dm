...
/datum/map/torch/get_map_info()
	. = list()
	. +=  "Вы находитесь на борту <b>ГЭК \"Факел\"</b>, космического корабля Экспедиционного Корпуса. Его главная цель - поиск разумной жизни, а также разведка систем встречаемых по пути. \
	Судно укомплектовано государственным персоналом из ЦПСС, а также нанятыми контратниками. \
	Эта часть космоса является неизученной, и находится далеко от территории ЦПСС. Вы можете встретить различные аванпосты или дрейфующие корабли, но ни одно подтверждённое государство не претендует на эти территории."
	return jointext(., "<br>")
